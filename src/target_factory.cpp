...
static uint64_t type2int(const std::string& type) {
  ...
  if (type == "DPUCZDX8G")
    ...
    ret = 9U;
  else if (type == "IPU_STRIX")
    ret = 10U;
  else if (type == "AutoDPU")
    ret = 11U;
  else if (type == "DPUCV3DX8G")
    ret = 12U;
  else
...
class TargetFactoryImp : public TargetFactory {
 public:
  ...
  std::vector<std::string> get_registered_target_names() const override {
    std::vector<std::string> names;
    for (const auto& name_fingerprint : map_name_fingerprint_) {
      names.push_back(name_fingerprint.first);
    }
    return names;
  }
